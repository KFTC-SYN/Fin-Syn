...
pipeline = {
    ...
    'ctgan': 'CTGAN/pipeline_ctgan.py',
    'tabpfgen': 'TabPFGen/pipeline_tabpfgen.py',
    'great': 'be_great/pipeline_great.py',
}
...
def eval_seeds(
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        elif sampling_method == "tabpfgen":
            shutil.copy2(parent_dir / "tabpfgen.obj", temp_config["parent_dir"])
        elif sampling_method == "great":
            shutil.copytree(parent_dir / "great_model", dir_ / "great_model", dirs_exist_ok=True)
            shutil.copy2(parent_dir / "great.obj", dir_)
        ...
        for sample_seed in range(n_datasets):
            ...
            if eval_type != 'real' and n_datasets > 1:
                ...
                # TabPFGen은 conda finsyn에서 실행해야 하므로 python3.11 버전으로 실행
                if sampling_method == "tabpfgen":
                    subprocess.run(['python3.11', f'{pipeline[sampling_method]}', '--config', f'{str(dir_ / "config.toml")}', '--sample', '--change_val'], check=True)
                else:
                    subprocess.run(['python3.9', f'{pipeline[sampling_method]}', '--config', f'{str(dir_ / "config.toml")}', '--sample', '--change_val'], check=True)
