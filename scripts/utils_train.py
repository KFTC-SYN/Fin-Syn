...
def get_model(
    ...
    if model_name == 'mlp':
        ...
        #############################################################################
        # 2026.1.19 ResNetDiffusion 지원: is_y_cond 파라미터 제거
        # - ResNetDiffusion의 __init__ 시그니처: (d_in, num_classes, rtdl_params, dim_t)
        # - MLPDiffusion의 __init__ 시그니처: (d_in, num_classes, is_y_cond, rtdl_params, dim_t)
        # - model_params는 딕셔너리이므로 복사본을 만들어 수정하여 원본 보존
        #############################################################################
        model_params_copy = model_params.copy()
        if 'is_y_cond' in model_params_copy:
            del model_params_copy['is_y_cond']
        model = ResNetDiffusion(**model_params_copy)
    else:
        raise ValueError(f"Unknown model type: {model_name}. Supported types: 'mlp', 'resnet'")
    return model
