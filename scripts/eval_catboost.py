...
def train_catboost(
    ...
    if D.is_regression:
        ...
        #############################################################################
        # 2026.1.19 클래스 불균형 처리: scale_pos_weight 추가
        # - 이진 분류에서 negative/positive 비율로 가중치 설정
        ...
        # 이진 분류인 경우 scale_pos_weight 계산
        if D.is_binclass:
            train_negative_count = np.sum(D.y['train'] == 0)
            train_positive_count = np.sum(D.y['train'] == 1)
            
            if train_positive_count > 0:
                scale_pos_weight = train_negative_count / train_positive_count
                classifier_kwargs['scale_pos_weight'] = scale_pos_weight
                print(f"클래스 불균형: Negative={train_negative_count}, Positive={train_positive_count}, scale_pos_weight={scale_pos_weight:.4f}")
        
        model = CatBoostClassifier(**classifier_kwargs)
