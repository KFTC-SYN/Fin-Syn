...
def train_mlp(
    ...
    if params is None:
        tuned_model_path = f"tuned_models/mlp/{Path(real_data_path).name}_cv.json"
        if os.path.exists(tuned_model_path):
            params = lib.load_json(tuned_model_path)
        else:
            print(f"Warning: {tuned_model_path} not found. Using default parameters.")
            params = None
