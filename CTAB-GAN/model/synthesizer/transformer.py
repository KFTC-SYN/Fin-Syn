...
class DataTransformer():
    ...
    def transform(self, data, ispositive = False, positive_list = None):
        ...
        if np.isnan(data).any() or np.isinf(data).any():
            print(f"Warning: Input data to transform contains NaN/inf. Replacing with zeros.")
            data = np.where(np.isnan(data) | np.isinf(data), 0.0, data)
        ...
        for id_, info in enumerate(self.meta):
            ...
            if np.isnan(current).any() or np.isinf(current).any():
                current = np.where(np.isnan(current) | np.isinf(current), 0.0, current)
            
            if info['type'] == "continuous":
                current = current.reshape([-1, 1])
                
                # current 값 클리핑 (float32 범위 내로 제한)
                current = np.clip(current, -1e6, 1e6)
                
                means = self.model[id_].means_.reshape((1, self.n_clusters))
                
                # covariances가 음수일 수 있으므로 절댓값 사용, 그리고 최소값 보장
                covariances = np.abs(self.model[id_].covariances_)
                stds = np.sqrt(np.maximum(covariances, 1e-8)).reshape((1, self.n_clusters))  # 최소 1e-8 보장
                
                # stds 안정성 체크
                stds = np.where(np.isnan(stds) | np.isinf(stds), 1.0, stds)
