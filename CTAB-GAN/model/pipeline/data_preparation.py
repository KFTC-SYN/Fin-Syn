...
class DataPrep(object):
    ...
    def __init__(self, raw_df: pd.DataFrame, categorical: list, log:list, mixed:dict, integer:list, type:dict, test_ratio:float):
        ...
        # integer_columns에 있는 컬럼을 숫자로 변환 (object 타입인 경우)
            ...
            for column in self.integer_columns:
                if column in self.df.columns:
                    if self.df[column].dtype == 'object':
                        ...
                        # 변환 실패한 값(empty 포함)은 -9999999로 처리
                        self.df[column] = self.df[column].fillna(-9999999)
                        # empty 문자열도 -9999999로 처리
                        if "empty" in list(self.df[column].values):
                            self.df[column] = self.df[column].apply(lambda x: -9999999 if x=="empty" else x)
                        # 숫자 타입으로 변환
