...
- **Seeding.** Four implementations (CTGAN, TVAE, CTAB-GAN, CTAB-GAN+) seed only their sampling step. Our wrappers (`train_sample_*.py`) additionally seed Python, NumPy, and PyTorch at the start of training. `scripts/check_generator_seeding.py` shows that two same-seed runs are then bit-identical on the CPU for all four and on the GPU for CTGAN and TVAE, while CTAB-GAN and CTAB-GAN+ still differ on the GPU because of non-deterministic convolution kernels (`exp/generator_seeding_check.json`, `exp/generator_seeding_check_cpu.json`). Their released data are therefore reproducible in distribution, not bit for bit, which is why we publish the data themselves and the spread across seeds.
