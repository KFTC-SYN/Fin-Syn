...
- **Seeding.** Four implementations (CTGAN, TVAE, CTAB-GAN, CTAB-GAN+) seed only their sampling step, so their training stage is not reproducible from the seed alone; we seed what the implementation exposes and report the spread across seeds instead of claiming bit-level reproducibility.
