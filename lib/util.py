...
def _normalize_for_comparison(data: Any) -> Any:
    """Convert tuples to lists for comparison (TOML doesn't support tuples)."""
    if isinstance(data, dict):
        return {k: _normalize_for_comparison(v) for k, v in data.items()}
...
def dump_config(config: Any, path: Union[Path, str]) -> None:
    ...
    # Note: TOML converts tuples to lists, so we normalize both for comparison
    loaded_config = load_config(path)
    assert _normalize_for_comparison(config) == _normalize_for_comparison(loaded_config)
