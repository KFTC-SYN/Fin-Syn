...
The sections below document an earlier iteration of this codebase, kept for reference. Its experiment directory (`exp/orig-micro-retry`) replicated a small pool of distinct rows before splitting at random, which is the construction error the paper analyses in Section 5.6; its results are not used in the paper and are not published here.
