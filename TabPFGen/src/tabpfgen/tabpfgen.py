...
from TabPFN.src.tabpfn import TabPFNClassifier, TabPFNRegressor
from TabPFN.src.tabpfn.constants import ModelVersion
...
class TabPFGen:
    ...
    def _infer_device(self, device: Union[str, torch.device, None]) -> torch.device:
        """
    ...
    def balance_dataset(
        ...
        if x_synthetic_list:
            ...
            # 2025.11.30
            # Use V2 model instead of V2_5 to avoid Hugging Face authentication requirement
            clf = TabPFNClassifier.create_default_for_version(ModelVersion.V2, device=self.device)
            clf.fit(X_train_valid, y_train_valid)
            
            # Predict in batches to avoid CUDA kernel errors for large number of samples
            BATCH_SIZE = 10000
            n_synth = x_synthetic_np.shape[0]
            probs_list = []
            
            for i in range(0, n_synth, BATCH_SIZE):
                end_idx = min(i + BATCH_SIZE, n_synth)
                batch_probs = clf.predict_proba(x_synthetic_np[i:end_idx])
    ...
    def generate_classification(
        ...
        for i in range(0, n_synth, BATCH_SIZE):
            end_idx = min(i + BATCH_SIZE, n_synth)
            batch_probs = clf.predict_proba(x_synth_np[i:end_idx])
            probs_list.append(batch_probs)
            if i % (BATCH_SIZE * 5) == 0:
                print(f"Predicting batch {i//BATCH_SIZE + 1}/{(n_synth + BATCH_SIZE - 1)//BATCH_SIZE}")
    ...
    def generate_regression(
        ...
        # 2025.11.30
        # Use V2 model instead of V2_5 to avoid Hugging Face authentication requirement
        regressor = TabPFNRegressor.create_default_for_version(ModelVersion.V2, device=self.device)
